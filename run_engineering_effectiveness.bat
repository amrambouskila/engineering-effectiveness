...
if not defined EE_PORT set "EE_PORT=5173"
set "PORT=%EE_PORT%"
...
echo.
echo ==============================
echo   ENGINEERING EFFECTIVENESS
echo   Effectiveness Radar
echo ==============================
echo.
echo   Frontend: %URL%
echo.
...
if %WAITED% GEQ %MAX_WAIT% (
    echo Warning: Service did not respond within %MAX_WAIT%s.
...
for /f "delims=" %%I in ('
    docker images --format "{{.Repository}}:{{.Tag}}" ^| findstr /I "engineering.effectiveness"
') do (
    echo Found image: %%I
    docker rmi -f "%%I" 2>nul
)
